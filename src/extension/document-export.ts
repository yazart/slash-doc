...
function exportBuiltInBlockToHtml(type: string, data: Record<string, unknown>): string {
  ...
  if (type === 'table' || type === 'confluenceTable') {
    ...
    return `<table>${rows.map((row, rowIndex) => {
      ...
      const rowHeight = typeof rowHeights[rowIndex] === 'number' && rowHeights[rowIndex] > 0 ? ` style="height:${rowHeights[rowIndex]}px"` : '';
      return `<tr${rowHeight}>${cells.map((cell, columnIndex) => {
        const tag = (headerRow && rowIndex === 0) || (headerColumn && columnIndex === 0) ? 'th' : 'td';
        const width = typeof columnWidths[columnIndex] === 'number' && columnWidths[columnIndex] > 0 ? ` style="width:${columnWidths[columnIndex]}px"` : '';
...
type ExportImageRegion = { id: string; number: number; x: number; y: number; width: number; height: number; description: string };
...
function exportImageAnnotationToHtml(data: Record<string, unknown>): string {
  ...
  const hotspots = annotation.annotations.map((region) => `<div class="slash-annotation-hotspot" tabindex="0" style="left:${region.x * 100}%;top:${region.y * 100}%;width:${region.width * 100}%;height:${region.height * 100}%"><div class="slash-annotation-tooltip"><strong>${region.number}</strong>${renderAnnotationMarkdown(region.description || 'No description')}</div></div>`).join('');
...
function exportImageAnnotationToMarkdown(data: Record<string, unknown>): string {
  ...
  const rows = annotation.annotations.map((region) => `| ${region.number} | ${escapeMarkdownTableCell(region.description || '—')} |`);
...
function renderAnnotationOverlay(regions: ExportImageRegion[], width = 1000, height = 1000): string {
  ...
  return regions.map((region) => {
    const x = region.x * width; const y = region.y * height; const regionWidth = region.width * width; const regionHeight = region.height * height;
    const badgeX = x + badgeRadius + strokeWidth; const badgeY = y + badgeRadius + strokeWidth;
...
function renderAnnotationHtmlRows(regions: ExportImageRegion[]): string {
  return regions.map((region) => `<tr><td>${region.number}</td><td>${renderAnnotationMarkdown(region.description || '—')}</td></tr>`).join('');
}
...
function normalizeExportImageAnnotation(data: Record<string, unknown>): ExportImageAnnotation {
  ...
  const image = value && typeof value.dataUrl === 'string' && value.dataUrl.startsWith('data:image/')
    ? { dataUrl: value.dataUrl, width: Math.max(1, getFiniteNumber(value.width, 1)), height: Math.max(1, getFiniteNumber(value.height, 1)), name: typeof value.name === 'string' ? value.name : 'annotated-image' }
    : null;
  const annotations = Array.isArray(data.annotations) ? data.annotations.filter(isRecord).flatMap((region, index) => {
    if (typeof region.id !== 'string') return [];
    return [{ id: region.id, number: index + 1, x: clampUnit(region.x), y: clampUnit(region.y), width: clampUnit(region.width), height: clampUnit(region.height), description: typeof region.description === 'string' ? region.description : '' }];
  }) : [];

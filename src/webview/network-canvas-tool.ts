...
function createNetworkData(data?: Partial<NetworkCanvasData>): NetworkCanvasData {
  const nodes = Array.isArray(data?.nodes) ? data.nodes.filter(isNode).map((item) => ({ ...item })) : [];
  const vlans = Array.isArray(data?.vlans)
    ? data.vlans.filter(isVlan).map((item) => ({
        ...item,
...
function isNode(value: unknown): value is NetworkNode {
  ...
  return (
    ...
    nodeTypes.some((type) => type.type === item.type) &&
...
function isConnection(value: unknown): value is NetworkConnection {
  ...
  return (
    ...
    (!item.lineType || lineTypes.includes(item.lineType))
  );
}
